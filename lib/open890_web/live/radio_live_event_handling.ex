defmodule Open890Web.Live.RadioLiveEventHandling do
  ...
  defmacro __using__(_) do
    quote location: :keep do
      ...
      def handle_event("ref_level_changed", params, %{assigns: %{radio_connection: connection}} = socket) do
      ...
      @impl true
      def handle_event("multi_ch", %{"is_up" => true} = params, %{assigns: %{radio_connection: connection}} = socket) do
      ...
      @impl true
      def handle_event("multi_ch", %{"is_up" => false} = params, %{assigns: %{radio_connection: connection}} = socket) do
      ...
      @impl true
      def handle_event("cmd", %{"cmd" => cmd} = _params, %{assigns: %{radio_connection: connection}} = socket) do
      ...
      def handle_event("scope_clicked", %{"x" => x} = _params, %{assigns: %{radio_connection: connection, active_receiver: active_receiver, band_scope_edges: {scope_low, scope_high}}} = socket) do
        new_frequency = x
        ...
        active_receiver
        |> case do
          :a -> connection |> Radio.cmd("FA#{new_frequency}")
          :b -> connection |> Radio.cmd("FB#{new_frequency}")
          vfo ->
